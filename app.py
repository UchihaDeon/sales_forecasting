...
with tab2:
    ...
    if 'df' in st.session_state:
        ...
        model_choice = st.selectbox("Choose forecasting model", ["ARIMA"])

        if model_choice == "ARIMA":
            order_p = st.number_input("ARIMA p", 0, 5, 1)
            order_d = st.number_input("ARIMA d", 0, 2, 1)
            order_q = st.number_input("ARIMA q", 0, 5, 1)
            ...
            if st.button("Run ARIMA Forecast"):
                ...
                # Plot
                from src.visualization import plot_actual_vs_forecast
    ...
    if 'df' in st.session_state:
        ...
        model_choice = st.selectbox("Choose forecasting model", ["ARIMA", "SARIMA"])
        ...
        if model_choice == "SARIMA":
            seasonal_p = st.number_input("Seasonal P", 0, 2, 1)
            seasonal_d = st.number_input("Seasonal D", 0, 2, 1)
            seasonal_q = st.number_input("Seasonal Q", 0, 2, 1)
            season_length = st.number_input("Season length (e.g. 7 for weekly)", 1, 30, 7)
            ...
            if st.button("Run SARIMA Forecast"):
                ...
                yhat = forecast(res, steps)
                future_index = pd.date_range(df.index.max() + pd.Timedelta(days=1),
            ...
            window_size = st.slider("Window size (days)", 7, 60, 30)
            ...
            if st.button("Run LSTM Forecast"):
                try:
                    ...
                    yhat, forecast_index = forecast_lstm(model, scaler, df['sales'], steps=steps,
...
with tab3:
    ...
    if 'df' in st.session_state:
        ...
        # Use last 30 days of actual sales as validation
        if len(df) >= 30:
...
with tab4:
    ...
    if 'df' in st.session_state:
        ...
        if forecasts:
            from src.visualization import plot_model_overlays
